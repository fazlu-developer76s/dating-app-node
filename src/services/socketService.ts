...
export const setupSocket = (io: Server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("sendMessage", async (message) => {
      console.log(message)
        console.log(`New message sent in room ${message}`);
      ...
      let roomKey = senderId < receiverId ? senderId + receiverId : receiverId + senderId;

      if (dataChat) {
        // Check if a message thread exists
        let existingMessage = await Message.findOne({ roomId: roomId });

        if (existingMessage) {
          // Update message array by appending a new object
          await Message.updateOne(
            { roomId: roomId },
            {
              $push: {
